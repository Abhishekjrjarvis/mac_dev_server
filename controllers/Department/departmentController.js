...
exports.renderStudentFormQuery = async (req, res) => {
  try {
    ...
    var valid_depart = await Department.findById(did)
      .select(
...
exports.render_one_student_form_section_query = async (req, res) => {
  try {
    ...
    dfs.form_section.push({
      ...
      section_value: "",
      form_checklist: [...dfs?.image_content],
    });
    res.status(200).send({
      message: "Explore One Department Student Form Section Query",
      access: true,
      section: dfs?.form_section,
    });
  } catch (e) {
    console.log(e);
  }
...
exports.render_one_student_form_section_enable_query = async (req, res) => {
  try {
    ...
    var depart = await Department.findById({ _id: did }).select(
      "student_form_setting"
    );
    var dfs = await DepartmentStudentForm.findById({
      _id: `${depart?.student_form_setting}`,
    })
      .select("form_section")
      .populate({
        path: "form_section.form_checklist",
...
exports.render_new_student_form_section_query = async (req, res) => {
  try {
    ...
    await dfs.save();
    res.status(200).send({
      message: "Explore One Department Form Section Query",
      access: true,
    });
  } catch (e) {
    console.log(e);
...
exports.render_new_student_form_checklist_query = async (req, res) => {
  try {
    ...
    await dfs.save();
    res.status(200).send({
      message: "Explore One Department Form Section Nested Checklist Query",
      access: true,
    });
  } catch (e) {
    console.log(e);
...
exports.render_dynamic_form_query = async (req, res) => {
  try {
    ...
    if (student?.student_form_flow?.flow === "INSTITUTE") {
        ...
        if (val?.section_key === "academic_details") {
          if (val?.section_visibilty == true) {
            for (var ele of val?.form_checklist) {
              ...
              if (
                student[`${ele?.form_checklist_key}`] === "Yes" ||
      ...
      res.status(200).send({
        message: "Explore One Student Institute Dynamic Form Query",
        access: true,
        result: [...head_arrays],
      });
      ...
      const all_check = await DepartmentStudentForm.findOne({
        department: student?.student_form_flow?.did,
      })
        .select("form_section")
        .populate({
          path: "form_section",
      ...
      head_arrays?.splice(0, 1);
      res.status(200).send({
        message: "Explore One Student Department Dynamic Form Query",
        access: true,
        result: [...head_arrays],
      });
      ...
      var app_name = await NewApplication.findById({
        _id: student?.student_form_flow?.did,
      }).select("applicationName");
      const all_check = await InstituteApplicationForm.findOne({
        application: student?.student_form_flow?.did,
      })
        .select("form_section")
        .populate({
          path: "form_section",
      ...
      head_arrays?.splice(0, 1);
      res.status(200).send({
        message: "Explore One Student Application Dynamic Form Query",
        access: true,
        result: [...head_arrays],
      });
    }
                ...
                if (ele?.form_checklist_visibility == true) {
                  for (let ads of ele?.nested_form_checklist) {
                    ...
                    custom.push({
                      ...
                      form_checklist_lable: ads?.form_checklist_lable,
                      form_checklist_typo: ads?.form_checklist_typo,
                      form_checklist_key_status: ads?.form_checklist_key_status,
                      form_checklist_typo_option_pl:
                        ads?.form_checklist_typo_option_pl,
                      form_checklist_required:
                        val?.section_key === "documents" ||
...
exports.render_dynamic_form_subject_list_query = async (req, res) => {
  try {
    ...
    if (student?.student_form_flow?.flow === "APPLICATION") {
      ...
      head_arrays?.splice(value, 0, {
        ...objs,
        key: "Selected Subjects",
        static_key: "selected_subjects",
      });
      res.status(200).send({
        message: "Explore One Student Application Dynamic Form Query",
        access: true,
        result: [...head_arrays],
        image_content: all_check?.image_content,
      });
    }
  } catch (e) {
    console.log(e);
  ...
  try {
    ...
    if (apk === "WEB") {
      if (flow === "INSTITUTE") {
        ...
        for (var ele of all_section) {
          for (var stu of ele?.form_checklist) {
            ...
            stu.form_checklist_required =
              ele?.section_key === "social_reservation_information_section"
                ? false
                : stu?.form_checklist_required;
          }
        }
        ...
        const depart_form = await DepartmentStudentForm.findOne({
          department: did,
        })
          .select("form_section")
          .populate({
        ...
        res.status(200).send({
          message: "Department Form Query",
          access: true,
          depart_form: all_section,
        });
      } else if (flow === "APPLICATION") {
        const app = await NewApplication.findById({ _id: did }).select(
          "subject_selected_group"
        );
        const app_form = await InstituteApplicationForm.findOne({
          application: did,
        })
          .select("form_section")
          .populate({
        ...
        var all_subjects = await SubjectGroup.find({
          _id: { $in: app?.subject_selected_group },
        })
          .populate({
          ...
          for (var val of ele?.subject_group_select) {
            ...
            for (var set of val?.optional_subject) {
              nums_select.push({
                ...
                form_checklist_typo_option_pl_optional: [
                  ...set?.optional_subject_options_or,
                ],
            ...
            for (var set of val?.fixed_subject) {
              nums_select.push({
                ...
                form_checklist_typo_option_pl: [...set?.fixed_subject_options],
        ...
        res.status(200).send({
          message: "Application Form Query",
          access: true,
          app_form: all_section,
        });
      }
    } else {
      if (flow === "INSTITUTE") {
        ...
        all_section[1].form_checklist.unshift(
          ...all_section?.[0]?.form_checklist
        );
        for (var ele of all_section) {
          for (var stu of ele?.form_checklist) {
            ele.form_checklist = ele?.form_checklist?.filter((qwe) => {
...
exports.render_add_form_image_query = async (req, res) => {
  try {
    ...
    if (flow === "INSTITUTE") {
      ...
      res
        .status(200)
        .send({ message: "Institute Form Image Query", access: true });
      var all_app = await NewApplication.find({
        admissionAdmin: ins?.admissionDepart?.[0],
      });
      var all_depart = await DepartmentStudentForm.find({
      ...
      res
        .status(200)
        .send({ message: "Department Form Image Query", access: true });
    } else if (flow === "APPLICATION") {
      const app_form = await InstituteApplicationForm.findOne({
        application: did,
      });
      ...
      res
        .status(200)
        .send({ message: "Application Form Image Query", access: true });
    }
  } catch (e) {
    console.log(e);
  }
...
exports.render_add_dynamic_textarea_field_query = async (req, res) => {
  try {
    const { flow, did, fsid, content, key } = req?.body;
    if (!flow && !did)
      return res.status(200).send({
