...
exports.retrieveAdmissionPayMode = async (req, res) => {
  try {
    ...
    aStatus.content = `Your admission is on hold please visit ${institute.insName}, ${institute.insDistrict}. with required fees & documents or contact institute if neccessory`;
...
exports.renderRemainingSetOffQuery = async (req, res) => {
  try {
    ...
    if (!sid && !rcid && !amount && !mode && !type)
      ...
      var admin_ins = await Admission.findById({ _id: `${apply?.admissionAdmin}` }).populate({
        path: "admissionAdminHead",
        select: "user",
      });
      var student = await Student.findById({ _id: sid }).populate({
    ...
    if(valid_price?.total > 0){
      ...
      if(filtered_card?.length > 1){
         var valid_remain_card = await RemainingList.findById({ _id: filtered_card?.[0]?.remain})
    const new_receipt = new FeeReceipt({ ...req.body });
    new_receipt.student = student?._id;
    new_receipt.application = apply?._id;
    new_receipt.finance = finance?._id;
