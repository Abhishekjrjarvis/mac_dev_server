...
exports.processAdmissionPayment = async (req, res, next) => {
  ...
  params[
    "CALLBACK_URL"
  ] = `http://18.205.27.165/api/api/v1/admission/callback/${uid}/apply/${aid}/ins/${iid}/finance/${fid}`;
        ...
        var post_req = https.request(options, function (post_res) {
          ...
          post_res.on("end", function () {
            ...
            if (status === "TXN_SUCCESS") {
              ...
              res.redirect(`http://18.205.27.165/userdashboard/${uid}`);
            } else {
...
const userAdmissionUpdated = async (
  ...
  try {
    ...
    institute.insBankBalance = institute.insBankBalance + parseInt(tx_adAmount);
    finance.financeBankBalance =
      finance.financeBankBalance + parseInt(tx_adAmount);
    await userText.save();
