...
exports.processApplicationPayment = async (req, res, next) => {
  ...
  params[
    "CALLBACK_URL"
  ] = `http://34.201.132.26/api/api/v1/application/callback/${uid}/apply/${aid}/ins/${iid}/finance/${fid}`;
        ...
        var post_req = https.request(options, function (post_res) {
          ...
          post_res.on("end", function () {
            ...
            if (status === "TXN_SUCCESS") {
              ...
              res.redirect(
                `http://34.201.132.26/user/${uid}/insjoinandapply/${iid}/application-apply/${aid}`
              );
            } else {
...
const userApplicationUpdated = async (
  ...
  try {
    ...
    institute.insBankBalance =
      institute.insBankBalance + parseInt(tx_apAmount);
