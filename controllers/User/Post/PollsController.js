...
exports.pollLike = async (req, res) => {
  try {
    ...
    if(user_session){
      if(!(user.answeredPoll.length >=1 && user.answeredPoll.includes(String(poll._id)))){
        ...
        user.answeredPoll.push(poll._id)
      }
      else{
        res.status(422).send({ message: "You're already voted"})
      }
    }
    else {
        res.status(401).send();
