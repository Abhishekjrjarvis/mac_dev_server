...
exports.retrieveNewElectionQuery = async(req, res) =>{
    try{
        ...
        res.status(200).send({ message: 'New Election Application will be available', status: true})
        elect.election_app_end_date = date_renew(elect.election_app_start_date, 'End').toISOString()
        elect.election_selection_date = date_renew(elect.election_app_end_date, 'Select').toISOString()
        elect.election_campaign_date = date_renew(elect.election_selection_date, 'Compaign').toISOString()
        elect.election_voting_date = date_renew(elect.election_campaign_date, 'Vote').toISOString()
        elect.election_result_date = date_renew(elect.election_voting_date, 'Result').toISOString()
        await elect.save()
        ...
        all_student?.forEach(async (ele) => {
            ...
            notify.notifyContent = `Apply from ${moment(elect?.election_app_start_date).format('LL')} to ${moment(elect?.election_app_end_date).format('LL')} , Voting Date ${moment(elect?.election_voting_date).format('LL')}.`;
...
exports.retrieveAllElectionQuery = async(req, res) => {
    try{
        ...
        .limit(limit)
        .skip(skip)
        .select('election_position election_app_start_date election_voting_date election_status')
        if(elect?.length > 0){
            res.status(200).send({ message: 'All Upcoming Election Event', elect: elect })
        }
        else{
            res.status(200).send({ message: 'No Upcoming Election Event', elect: [] })
        }
    }
    catch(e){
        console.log(e)
...
exports.retrieveOneElectionQuery = async(req, res) => {
    try{
        ...
        const elect = await Election.find({_id: eid})
        .select('election_position election_app_start_date election_app_end_date election_selection_date election_compaign_date election_result_date election_voting_date election_status')
        .populate({
            ...
            select: 'election_candidate_status election_result_status election_vote_receieved election_tag_line election_description'
        })
        res.status(200).send({ message: 'One Election Event Process Query ', elect: elect })
    }
    catch(e){
        console.log(e)
    }
...
exports.retrieveApplyElectionQuery = async(req, res) => {
    try{
        ...
        await elect.save()
        res.status(200).send({ message: 'One Election Event Process Query ', apply_elect: true })

        members?.forEach( async (ele) => {
            const student_support = await Student.findById({_id: `${ele}`})
            .select('user notification')
            const notify = new StudentNotification({});
            const user = await User.findById({_id: `${student_support?.user}`}).select('activity_tab deviceToken')
            notify.notifyContent = `You have been choosen as supporting member for ${elect?.election_position} by ${student.studentFirstName} ${student.studentMiddleName ? student.studentMiddleName : ''} ${student.studentLastName}. (If you have not supporting please contact with respectiv department)`;
...
exports.retrieveStatusElectionQuery = async(req, res) =>{
    try{
        ...
        var elect = await Election.findById({_id: eid})
        var depart = await Department.findById({_id: `${elect?.department}`})
        ...
        const student = await Student.findById({_id: sid}).select('user notification')
        const notify = new StudentNotification({});
        const user = await User.findById({_id: `${student?.user}`}).select('activity_tab deviceToken')
        notify.notifyContent = `Apply from ${moment(elect?.election_app_start_date).format('LL')} to ${moment(elect?.election_app_end_date).format('LL')} , Voting Date ${moment(elect?.election_voting_date).format('LL')}.
        Your application for ${elect?.election_position} is ${status} by authorities`;
        notify.notifySender = depart._id;
        notify.notifyReceiever = user._id;
        notify.electionId = elect?._id;
        notify.notifyType = "Student";
