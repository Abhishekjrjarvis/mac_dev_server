...
const hostelModifyReceiptData = (args1, args2, selectedApplicationId = "") => {
  ...
  let studentInfo = {
    ...
    batchName:
      ...
      "N/A",
    hostelUnit: args2?.application?.applicationUnit?.hostel_unit_name ?? "N/A",
    room: "N/A",
    bednumber: "N/A",
    ...
    //   : "",
    receiverSignature:
      args2?.application?.hostelAdmin?.site_info?.[0]?.cashier_signature ?? "",
    receiverName:
      args2?.application?.hostelAdmin?.site_info?.[0]?.cashier_name ?? "",
  };
...
const normalHostelReceiptData = async (receiptId, appId) => {
  const data = await renderOneFeeReceiptUploadQuery(receiptId);
  const { institute, studentInfo, paymentReceiptInfo } =
    await hostelModifyReceiptData(data?.all_remain, data?.receipt, appId);

  return { institute, studentInfo, paymentReceiptInfo };
};
module.exports = normalHostelReceiptData;
