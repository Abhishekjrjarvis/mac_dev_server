...
exports.fee_reordering_direct_student_payload_exist_query = async (
  ...
  //     });
  //     var price = ref?.amount ? parseInt(ref?.amount) : -1;
  //     if (price >= 0 && ref?.appId && ref?.fee_struct) {
  //       var apply = await NewApplication.findById({ _id: ref?.appId });
  //       var admission = await Admission.findById({
  //         _id: `${apply?.admissionAdmin}`,
  //       });
  ...
  //         fee_structure: fee_structure,
  //       };
  //       var new_data = await render_new_fees_card(student?._id, apply?._id, fee_structure?._id)
  //       var new_remainFee = new RemainingList.findById({ _id: new_data?.card })
  //       var nest_card = new NestedCard.findById({ _id: new_data?.app_card })
  //       var nest_gov = new NestedCard.findById({ _id: new_data?.gov_card})
  //       if (price >= 0 && !is_install) {
  //         for (var nest of ref?.remain_array) {
  //           const nestPrice = nest?.amount ? parseInt(nest?.amount) : -1;
  //           if (nestPrice == 0) {
